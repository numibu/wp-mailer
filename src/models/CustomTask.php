...
class CustomTask /* extends Model */ {
    ...
    static $tableName = 'wp_list_of_mailer_custom';
    ...
    public static function getAll( $conf=array() )
    {
        ...
        if( count($conf) === 0 ){
            ...
            $sql = "SELECT $fields FROM $table WHERE mailer_id = $userID AND is_send = false ORDER BY addressee_name DESC";
        ...
        array_walk($result, 'static::creatCustomTask');
    ...
    public static function getTasksOfMail($mail, $conf=array())
    {
        ...
        if( count($conf) === 0 ){
            $fields = static::key_implode(', ', static::getAttrNames());
            $sql = "SELECT $fields FROM $table WHERE addressee_mail = '$mail' AND mailer_id = $userID  AND is_send = false ";
            $result = $wpdb->get_results($sql);
        }else{
            $fields = static::key_implode(', ', $conf);
            $sql = "SELECT $fields FROM $table WHERE addressee_mail = '$mail' AND mailer_id = $userID  AND is_send = false ";
    ...
    protected function update()
    {
        ...
        $res = $wpdb->update(static::$tableName, 
                            array(  'addressee_name' => $this->addressee_name,
                                    'addressee_mail' => $this->addressee_mail,
    ...
    protected function init($id_item,  $objSTD = NULL) {
        ...
        if ($res !== NULL && count($res) > 0) {
            $obj = $res[0];
            $this->id = $obj->id;
            $this->addressee_name = $obj->addressee_name;
            $this->addressee_mail = $obj->addressee_mail;
    ...
    protected static function creatCustomTask(&$val, $key)
    {
        $val = new CustomTask(true, $val);
    }
    ...
    protected static function getAttrNames() {
        $class = new \ReflectionClass('\mailer\models\CustomTask');
        $names = [];
        foreach ($class->getProperties(\ReflectionProperty::IS_PUBLIC) as $property) {
            if (!$property->isStatic()) {
